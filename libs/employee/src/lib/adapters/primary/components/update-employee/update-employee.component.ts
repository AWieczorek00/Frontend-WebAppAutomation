...
import { GET_ONE_EMPLOYEE_DTO_PORT, GetOneEmployeeDtoPort } from '../../../../application/ports/secondary/dto/get-one-employee.dto-port';
...
import { UPDATE_EMPLOYEE_COMMAND_PORT, UpdateEmployeeCommandPort } from '../../../../application/ports/primary/command/update-employee.command-port';
...
export class UpdateEmployeeComponent {
  ...
  //     )
  //   );
  readonly employee$: Observable<EmployeeQuery> = this._getOneEmployeeDtoPort.getOne(this.data.individualId)
  constructor(
    ...
    private _activatedRoute: ActivatedRoute, @Inject(UPDATE_EMPLOYEE_COMMAND_PORT) private _updateEmployeeCommandPort: UpdateEmployeeCommandPort
  ) {
    this.employee$.subscribe((employee) => this.employee.setValue(
      {
  ...
  onEmployeeSubmitted(employee: FormGroup) {
    this._updateEmployeeCommandPort.update(new UpdateEmployeeCommand(
